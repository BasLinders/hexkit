...
# --- Check authentication and email domain ---
if authentication_status:
    email = config['credentials']['usernames'][username]['email']
    allowed_domains = ['yourdomain.com', 'anotherdomain.org']  # Replace with your allowed domains

    if any(email.endswith(f'@{domain}') for domain in allowed_domains):
        # --- Main Page UI (only shown if email domain is allowed) ---
        st.set_page_config(
            page_title="HEXKIT",
            page_icon="📈",
        ...
        st.error('Access denied: Your email domain is not allowed.')
        authenticator.logout('Logout', 'main')
